...
def score_result(result: DigestResult, expectations: Dict[str, object]) -> Dict[str, object]:
    ...
    unsupported_terms = [str(value).casefold() for value in expectations.get("unsupported_terms", [])]
    ...
    for candidate in candidates:
        ...
        if scores is not None:
            scores["stability"]["slug_stability"] = (
                1.0 if len(slug_sets) < 2 else round(len(set.intersection(*slug_sets)) / len(set.union(*slug_sets)), 4)
            )
        preset = resolve_preset(candidate.preset)
        runs.append(
            {
