            ...
            return DigestDecision(
                topic_updates=[
                    TopicDigest(
                        slug="testing",
                        title="Testing",
                        routing_description="Use this skill when validating the current implementation changes.",
                        summary="Captures the validation flow and the checks that should follow each implementation change.",
                        key_points=[
                            "Run the checks after each change.",
                            "Keep the validation loop close to the code path you modified.",
                        ],
                        workflow_notes=["Re-open the source material when a check fails unexpectedly."],
                        references=[chunk.source_ref],
                    )
                ],
                should_continue=False,
        ...
        return DigestDecision(
            topic_updates=[
                TopicDigest(
                    slug="architecture",
                    title="Architecture",
                    routing_description="Use this skill when tracing architecture decisions across the corpus.",
                    summary=(
                        "Architecture notes from batch {batch} that preserve the design details for later reuse."
                    ).format(batch=request.batch_number),
                    key_points=[
                        "Architecture point {batch}".format(batch=request.batch_number),
                        "Reconcile architecture changes with earlier finalized guidance.",
                    ],
                    workflow_notes=["Validate the architecture note against the cited source chunk."],
                    references=[chunk.source_ref],
                )
            ],
            should_continue=False,
...
def test_document_digester_merges_reopened_topics_across_finalized_clusters(tmp_path: Path) -> None:
    ...
    assert "Architecture notes from batch 1 that preserve the design details for later reuse." in architecture.summary
    assert "Architecture notes from batch 3 that preserve the design details for later reuse." in architecture.summary
    assert architecture.key_points == [
    ...
    assert "Architecture notes from batch 1 that preserve the design details for later reuse." in writer.topic_summaries[-1]["architecture"]
    assert "Architecture notes from batch 3 that preserve the design details for later reuse." in writer.topic_summaries[-1]["architecture"]
